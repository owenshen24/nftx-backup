...
export class Web3Enabled {
  ...
  async sendTxWithTokenAndNFT(func, token, nft, to, amount, gasLimit, _onTxHash, _onReceipt, _onError) {
    const maxAllowance = new BigNumber(2).pow(256).minus(1).integerValue().toFixed();
    const state = this.state;
    const allowance = new BigNumber(await token.methods.allowance(state.address, to).call());
    if (allowance.gte(amount)) {
      await this.sendTxWithNFT(func, nft, to, gasLimit, _onTxHash, _onReceipt, _onError);
    }
    else {
      return this.sendTx(token.methods.approve(to, maxAllowance), async() => {
        await this.sendTxWithNFT(func, nft, to, gasLimit, _onTxHash, _onReceipt, _onError);
      }, ()=>{},()=>{});
    }
  }
